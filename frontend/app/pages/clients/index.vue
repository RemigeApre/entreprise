    ...
    <div class="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
      ...
      <template v-else>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
          <span class="text-xs text-[#2c2419]/60">
            <strong class="text-[#2c2419]">{{ clients.length }}</strong> client{{ clients.length > 1 ? 's' : '' }}
          </span>
          <UInput
            v-if="clients.length"
            v-model="search"
        ...
        <div v-if="!clients.length" class="text-center py-12">
          <UIcon name="i-lucide-building" class="size-10 text-stone-300 mx-auto mb-3" />
          <p class="text-stone-500">Aucun client pour le moment</p>
          <p class="text-xs text-stone-400 mt-1">Les prospects avec le statut "Client" apparaitront ici</p>
          <UButton
        ...
        <div v-else-if="search && !filteredClients.length" class="text-center py-12">
          <p class="text-stone-500">Aucun client pour "{{ search }}"</p>
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3">
          <NuxtLink
            ...
            class="block group"
          >
            <div class="relative flex flex-col gap-2.5 rounded-xl border border-emerald-200/60 p-4 transition-all h-full hover:border-emerald-300 hover:bg-emerald-50/30">
              <!-- Header -->
              <div class="flex items-start justify-between gap-2">
                <div class="flex-1 min-w-0">
                  <h3 class="text-sm font-semibold text-[#2c2419] truncate group-hover:text-emerald-700 transition-colors">
